...
function sbsdkecho {
    echo "[ScanbotBarcodeScannerSDK] $1"
}
...
function processDebugSymbols {
    ...
    if [ -n "$SCRIPT_INPUT_FILE_0" ]; then
        ...
        symbolsSourceBinary="$symbolsSourcePath/Contents/Resources/DWARF/ScanbotBarcodeScannerSDK"
...
function runScript {
    frameworkFolder=$1
    frameworkBinary="$frameworkFolder/ScanbotBarcodeScannerSDK"
    processFiles "$frameworkFolder"
    strippedArchitectures=$(stripBinary "$frameworkBinary")
    
    if [[ -n "$strippedArchitectures" ]]; then
        sbsdkecho "Removed architectures from ScanbotBarcodeScannerSDK framework: $strippedArchitectures"
...
folder="./ScanbotBarcodeScannerSDK.framework"
runScript "$folder"
